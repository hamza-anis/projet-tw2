...
function catsToOptionsHTML($liste){
    $res="";
    foreach($liste as $info){
        $res .= "<option value='{$info['name']}'>{$info['name']} </option>\n";
    }
    return $res;
}
function eventsToOptionsHTML($liste){
    $res="";
    foreach($liste as $info){
        $res .= "<option value='{$info['event']}'>{$info['event']} \n le {$info['date']}\n par {$info['auteur']} participants: {$info["Participant(s)"]} </option>\n";
    }
    return $res;
}
